...
public class DPfMD extends PApplet {
...
public void setup() {
 ...
 background(white);
 factor=(ADCmax-ADCmin)/(0-800.0f);
 offs=800.0f-ADCmin/factor;
 println(factor);
 println(offs);
...
public void dibuja(){
 ...
 int y=0;
  for (int i = 1; i < maxgraf-1; i++) {
    int j=(i+ng) % maxgraf;
    y=PApplet.parseInt((grafica[j]/factor)+offs);
    line(i-1,yant,i,y);
    yant=y;
  }
}

public void leePuertoBin(){
...
public void Quit(){exit();}	
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "DPfMD" };
    if (passedArgs != null) {
